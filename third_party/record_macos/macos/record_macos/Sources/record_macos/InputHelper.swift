...
func getInputSampleRates(deviceID: AudioDeviceID) -> [Int] {
  ...
  // Mosh patch: a virtual driver (Apowersoft) reports the range 0...DBL_MAX,
  // and Int(DBL_MAX) traps (SIGILL), killing the app when Settings lists the
  // inputs. Keep only rates that are real, positive and fit in an Int.
  var rates = Set<Int>()
  for range in ranges {
    for bound in [range.mMinimum, range.mMaximum] {
      if bound.isFinite, bound >= 1, bound <= Double(Int32.max) { rates.insert(Int(bound)) }
    }
  }
  return rates.sorted()
}
