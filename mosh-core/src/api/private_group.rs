...
//! `OnceLock<Mutex<Option<PrivateGroupRuntime>>>` plus a cached
//! `load_error`. `ensure_runtime()` constructs the singleton on first
//! call, then locks and borrows it. Each public function calls
...
//! DM/channel facades get. No per-facade Moss load.
//!
...
/// saved groups from the encrypted store.
