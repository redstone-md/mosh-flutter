...
- macOS app, `build`: on macOS with Xcode, `node scripts/moss-prepare.mjs && flutter build macos --release` -> `build/macos/Build/Products/Release/mosh.app` (universal). The Xcode "Moss Runtime" copy phase bundles `libmoss.dylib` into `Contents/MacOS/` where `moss_runtime.rs` finds it next to the executable — `moss-prepare` must run first or the build fails fast. The Rust core's vendored opus builds through autotools: `brew install autoconf automake libtool` is a one-time local prerequisite (CI installs it in `.github/actions/setup`).
