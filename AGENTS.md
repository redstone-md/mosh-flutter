...
1. `node scripts/moss-prepare.mjs` builds the Go FFI (`go build -buildmode=c-shared` from `moss/`, GOTOOLCHAIN pin `go1.25.9`) into ignored `moss-runtime/moss.dll` (or `.so`/`.dylib`). On darwin it builds one slice per GOARCH (arm64, amd64) and `lipo`s them into a universal `libmoss.dylib`. This is the canonical candidate path.
...
- Flutter, `test`: `flutter test` (widget/unit; tests override `gatewayProvider` with `ScriptableGateway` from `test/support/`, and mount a screen with `pumpScreen` / `pumpRoute` from `test/support/pump.dart` instead of building their own `MaterialApp`)
...
- macOS app, `build`: on macOS with Xcode, `node scripts/moss-prepare.mjs && bash scripts/opus-prepare-macos.sh && flutter build macos --release` -> `build/macos/Build/Products/Release/mosh.app` (universal). The Xcode "Moss Runtime" copy phase bundles `libmoss.dylib` into `Contents/MacOS/` where `moss_runtime.rs` finds it next to the executable — `moss-prepare` must run first or the build fails fast. `opus-prepare-macos.sh` provisions the universal `libopus.a` audiopus_sys links (its vendored build cannot cross-compile the x86_64 slice). The Rust core's vendored opus path additionally needs autotools: `brew install autoconf automake libtool` is a one-time local prerequisite (CI installs it in `.github/actions/setup`).
- macOS installer, `package`: `bash scripts/macos-package.sh` -> `build/macos-dist/Mosh_<version>_universal.dmg` (+ `.sha256`). Unsigned (ad-hoc): Gatekeeper warns on first launch; README documents the "Open Anyway" flow. The DMG is the only shippable macOS artifact, built by `.github/workflows/build-macos.yml`.
