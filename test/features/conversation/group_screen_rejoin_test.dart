// Widget test for the group `needs_rejoin` inline-error -- the 1-в-1 port
// of React's `<div className="inline-error" role="alert">` fragment
// (ActiveChatPanes.tsx L355-360), wired into GroupScreen between the
// CryptoNoticeBanner (GroupNotice) and ConversationTools (matching React's
// `afterHeader` order). Asserts the inline-error renders with the localized
// title (with the appended period React adds via `<strong>{title}.</strong>`)
// + body when `needsRejoin` is true, and does NOT render when false. Mirrors
// the seed/override idiom of `group_screen_notice_test.dart` (override
...
void main() {
  ...
  // -- the title ARB value ("Group out of sync") has NO trailing period;
  // React appends it. The widget renders the title + "." + " " + body as a
  // single `Text.rich`, so its `toPlainText()` is the concatenation
  // "Group out of sync. This group missed...". `find.text` matches a
  // `Text.rich` by its span's `toPlainText()`, so we assert on the full
  // string (the bold span carries the period React appends via
  // `<strong>{title}.</strong>`).
