  ...
  group('call_frame_transport', () {
    test('callDrainFrames base64-encodes each raw frame from the gateway', () async {
      final gateway = ScriptableGateway()
        ..seedCallFrames([Uint8List.fromList([1, 2, 3]), Uint8List.fromList([4, 5])]);
    ...
    test('callDrainFrames passes sessionId and callId through to the gateway', () async {
      ...
      expect(gateway.lastCall(GatewayMethod.callDrainFrames)?.arg<String>('sessionId'), 's1');
      expect(gateway.lastCall(GatewayMethod.callDrainFrames)?.arg<String>('callId'), 'c1');
    });
    ...
    test('sendFrameBytes forwards the raw bytes to the gateway', () async {
      ...
      expect(gateway.lastCall(GatewayMethod.callSendFrame)?.arg<String>('sessionId'), 's');
      expect(gateway.lastCall(GatewayMethod.callSendFrame)?.arg<String>('callId'), 'c');
      expect(gateway.argValues<Uint8List>(GatewayMethod.callSendFrame, 'frame').single, Uint8List.fromList([9, 9, 9]));
    });
