...
void main() {
  group('frame_crypto', () {
    test('seals and opens a frame with the same key', () async {
      ...
      final sealed = await sealFrame(key, PREFIX_B64, BigInt.from(7), CALLER_DIRECTION_BIT, payload);
    ...
    test('rejects a seq that would overflow the 63-bit value space (no silent wrap)', () async {
      final key = await importCallKey(KEY_B64);
      // 2^63 masks down to 0 -- sealing it would silently reuse seq 0's nonce.
      expect(
        () => sealFrame(key, PREFIX_B64, BigInt.one << 63, CALLER_DIRECTION_BIT, Uint8List.fromList([1])),
        throwsArgumentError,
      );
      expect(
        () => sealFrame(key, PREFIX_B64, BigInt.from(-1), CALLER_DIRECTION_BIT, Uint8List.fromList([1])),
        throwsArgumentError,
      );
    });
  });
