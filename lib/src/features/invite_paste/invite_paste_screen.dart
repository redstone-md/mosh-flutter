// Invite Paste route wrapper -- thin Scaffold + SafeArea over
// [OnboardStepFrame], which composes the embeddable [OnboardJoinStep]. The
// step CONTENT (body, invite field with live 3-state detection badge,
// Connect button, accepted-session/error text, all state + the success
...
/// the success navigation live in [OnboardJoinStep]; this screen only
/// decides Back routing and forwards the deep-link seed.
///
...
/// route reads `state.extra` (the raw URI string) and forwards it to
/// [OnboardJoinStep.initialInviteUri]. Null by default, so the existing
/// no-arg widget test and in-app navigation keep working unchanged.
