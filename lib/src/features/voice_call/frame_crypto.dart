...
/// `[0, SEQ_VALUE_MASK]`. Returns the wire frame `[seq][ciphertext+tag]`
/// (cryptography's `SecretBox` keeps the tag separate from `cipherText`,
/// so we re-concatenate for the wire format).
...
/// Opens a sealed wire frame. Returns `null` on any failure (too-short
/// frame, missing tag, GCM auth mismatch). `parsed.ciphertext` is
/// `[ciphertext][16-byte tag]`; the tag is always the last 16 bytes, so
/// we split, rebuild a `SecretBox`, and decrypt.
