...
/// Discriminator for the two warning branches: the persistence-not-encrypted
/// branch (`unavailable`) and the gateway-threw branch (`error`).
...
/// `nativeRuntimeStatusProvider` (the gateway seam, ADR 0013).
/// browser-demo link-mode -> null;
...
/// the `error` warning without rethrowing.
final persistenceWarningProvider =
  ...
  // `if (!status || status.moss.link_mode === "browser-demo") return null`.
  if (status.moss.linkMode == 'browser-demo') {
    return null;
  }
  // `if (status.persistence.available && status.persistence.encrypted_at_rest) return null`.
  if (status.persistence.available && status.persistence.encryptedAtRest) {
    return null;
  }
  // `reason = status.persistence.error ? ` Reason: ${error}` : ""`.
  return PersistenceWarning(
